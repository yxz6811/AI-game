...
**Prerequisites**: spec.md、plan.md、research.md、data-model.md、contracts/、quickstart.md（均已就绪，基于 PRD2.md）
...
> **本版本变更记录（对齐 `/speckit-analyze` 发现的问题）**：本次相对上一版本新增 5 条任务（原 140 条 → 现 145 条），并修改了 4 条既有任务的描述/依赖标注，具体见下方对应任务旁的标记。任务 ID 因插入新任务而整体重新编号（Phase 7 起）。
> - **C1（回归脚本覆盖不对称）**：Phase 7、Phase 8 新增回归脚本执行任务（新 T106、T134）
> - **C2（第四层隔离验证不对称）**：Agent Arena、Replay-to-Skill 子方向新增"关闭后前三层不受影响"验证任务（新 T116、T124），与已有的 Shadow Observer 隔离验证（T131）对齐
> - **U1（MUST NOT 型边界需求缺少审查任务）**：T016（Stage Gate 清单）描述补充 FR-008 排除项核对；新增 T100（FR-036 代码审查）
> - **I1（Phase 6/7 缺少显式 Gate 依赖标注）**：T072、T090 补充"依赖 Gate 2A/2B（T049/T070）"标注
> - **U2（Edge Cases 保留已被 PRD2 解答的开放问句）+ 对应 Runbook 任务**：T027 描述补充明确的回滚策略
...
```
airi/                          # Project AIRI 固定版本/commit（git submodule），仅在明确必要处做最小侵入式修改
hackathon-services/            # 本项目新增的外围服务，均通过 @proj-airi/server-sdk 接入 AIRI 事件总线
demo/                          # 演示脚本、Runbook、版本矩阵、录屏、交付说明
```
...
- [ ] T001 创建仓库骨架：`airi/`（AIRI 子模块占位）、`hackathon-services/`、`demo/` 三个顶层目录 — 优先级：P0，预估：0.5h
- [ ] T002 [P] 将 Project AIRI 引入为 git submodule 于 `airi/` — 优先级：P0，预估：0.5h
- [ ] T003 确认并锁定 AIRI 版本/commit，记录到 `demo/version-matrix.md`（对应 PRD2 Phase 0 范围冻结） — 优先级：P0，预估：1h（依赖 T002）
- [ ] T004 确认目标演示机器、Minecraft 服务器版本、模型提供商、Demo 世界与各阶段负责人，记录到 `demo/version-matrix.md` — 优先级：P0，预估：1.5h
- [ ] T005 在目标机器安装 pnpm 并执行 `pnpm i` 完成 AIRI workspace 依赖安装 — 优先级：P0，预估：0.5h（依赖 T003）
- [ ] T006 [P] 搭建受控 Minecraft Java 版测试服务器与专用测试世界 — 优先级：P0，预估：1.5h
- [ ] T007 [P] 初始化 `hackathon-services/` 下各子包的 package.json/tsconfig 骨架（game-adapter/dst-bridge/voice-orchestration 等空目录先占位） — 优先级：P1，预估：1h
- [ ] T008 [P] 搭建 CI 骨架（push 时对 `hackathon-services/` 跑 lint + typecheck；不对 `airi/` 做修改） — 优先级：P1，预估：1h
- [ ] T009 编写 `demo/runbook.md` 骨架（故障排查清单模板，L1-07 的载体） — 优先级：P0，预估：1h
...
## Phase 4: User Story 2 - 全双工实时语音打断与重规划 (Priority: P2)

**Goal**: 玩家可随时打断 AI 发言与正在执行的游戏任务，系统能取消/重规划并响应新输入。

**Independent Test**: 20 次打断测试 ≥18 次正确停止旧语音；10 次"停止/改派"测试 ≥8 次正确取消；不影响第一层演示。

- [ ] T034 [US2] 调研并确认 AIRI 现有 VAD/STT/TTS 模块的具体接入方式与配置项（对应 `unspeech` 统一音频代理），于 `hackathon-services/voice-orchestration/README.md` — 优先级：P0，预估：2h
- [ ] T035 [US2] 接入持续麦克风音频流，验证 VAD 可区分说话/静音/噪声状态（V2-01，MUST） — 优先级：P0，预估：3h（依赖 T034）
- [ ] T036 [US2] 验证流式 STT 提供增量文本（无需等待完整句子），于 `hackathon-services/voice-orchestration/src/stt.ts`（V2-02，MUST） — 优先级：P0，预估：2h（依赖 T035）
- [ ] T037 [US2] 验证 TTS 支持分段播放（无需等待完整回复生成完毕），于 `hackathon-services/voice-orchestration/src/tts.ts`（V2-02，MUST） — 优先级：P0，预估：2h（依赖 T036）
- [ ] T038 [US2] 研究并设计复用 `reflex-manager.ts` 抑制信号机制的打断信号传播路径，于 `hackathon-services/voice-orchestration/src/interrupt-design.md`（research.md §2） — 优先级：P0，预估：2h
- [ ] T039 [US2] 实现打断控制器：玩家开口时停止当前 TTS 播放并发送打断事件，于 `hackathon-services/voice-orchestration/src/barge-in-controller.ts`（V2-03 机制本身，MUST） — 优先级：P0，预估：4h（依赖 T037, T038）
- [ ] T040 [US2] 实现任务取消令牌传递：打断事件同时触发 Action 层任务取消，于 `hackathon-services/voice-orchestration/src/task-cancellation.ts`（V2-04 机制本身，MUST） — 优先级：P0，预估：3h（依赖 T039）
- [ ] T041 [US2] 运行 20 次打断测试，统计正确停止旧语音次数（V2-03 量化验收，目标 ≥18/20，对应 SC-004"目标"级） — 优先级：P1，预估：2h（依赖 T039）
- [ ] T042 [US2] 运行 10 次"停止/改派"测试，统计正确取消次数（V2-04 量化验收，目标 ≥8/10，对应 SC-005"目标"级） — 优先级：P1，预估：1.5h（依赖 T040）
- [ ] T043 [US2] 实现回声消除/输出抑制策略，避免 AI 自身 TTS 被再次识别为输入，于 `hackathon-services/voice-orchestration/src/aec.ts`（V2-05，MUST） — 优先级：P0，预估：3h（依赖 T037）
- [ ] T044 [US2] 连续 10 分钟对话测试，确认无自激对话现象（V2-05 验收） — 优先级：P1，预估：1.5h（依赖 T043）
- [ ] T045 [US2] 实现各阶段（VAD/STT/LLM/TTS/行动）延迟埋点与结构化日志/看板，于 `hackathon-services/voice-orchestration/src/latency-dashboard.ts`（V2-06，MUST） — 优先级：P0，预估：3h
- [ ] T046 [US2] 人为制造一次慢响应，验证可通过看板定位到具体阶段（V2-06 验收） — 优先级：P1，预估：1h（依赖 T045）
- [ ] T047 [P] [US2] 保留文本输入作为语音故障的现场降级通道（§5.3） — 优先级：P1，预估：1.5h
- [ ] T048 [US2] 端到端联调：完整全双工语音打断+取消场景（quickstart.md 场景 B） — 优先级：P0，预估：3h（依赖 T041, T042, T044, T046）
- [ ] T049 [US2] Gate 2A 判定：确认全双工语音的打断、取消和重规划可现场复现 — 优先级：P0，预估：1h（依赖 T048）
- [ ] T050 [P] [US2] 编写语音编排模块的自动化测试（打断信号传播路径），于 `hackathon-services/voice-orchestration/tests/` — 优先级：P1，预估：2.5h

**Checkpoint**：第一层 + 第二层 A 均应可独立正常工作，Gate 2A 通过。
...
- [ ] T051 [US3] 按 `contracts/game-adapter-contract.md` 定义 Adapter 契约的 TypeScript 类型（observe/act/events/capabilities/health），于 `hackathon-services/game-adapter/src/contract/types.ts` — 优先级：P0，预估：2.5h
- [ ] T052 [US3] 定义统一 GameState Schema（角色位置/生命/饥饿/精神值/附近实体/资源/危险/任务状态），于 `hackathon-services/game-adapter/src/contract/game-state.ts`（G2-01，MUST） — 优先级：P0，预估：2h（依赖 T051）
- [ ] T053 [US3] 定义统一 Action Schema（follow/move/collect/interact/stop/say），于 `hackathon-services/game-adapter/src/contract/actions.ts`（G2-02，MUST） — 优先级：P0，预估：1.5h（依赖 T051）
- [ ] T054 [US3] 实现 Minecraft Adapter：包装 `airi/services/minecraft` 的 `action-registry.ts`/`task-executor.ts`，不修改其内部逻辑，于 `hackathon-services/game-adapter/src/minecraft-adapter/index.ts`（G2-03，MUST） — 优先级：P0，预估：4h（依赖 T052, T053）
- [ ] T055 [US3] 实现 Minecraft Adapter 的 `observe()`：从既有 Perception 层状态映射到统一 GameState — 优先级：P0，预估：2h（依赖 T054）
- [ ] T056 [US3] 实现 Minecraft Adapter 的 `events()`：转发既有 Perception 事件总线事件并做形状归一化 — 优先级：P0，预估：2h（依赖 T054）
- [ ] T057 [US3] 实现 Minecraft Adapter 的 `capabilities()`/`health()` — 优先级：P0，预估：1.5h（依赖 T054）
...
- [ ] T083 [US4] 将推理桥接接入 `airi/services/minecraft` 的 `reflex-manager.ts`（最小侵入式修改，合并前打 Tag 便于回滚） — 优先级：P0，预估：3h（依赖 T082）
...
```bash
# User Story 2（语音）与 User Story 3（Adapter+DST）可并行推进：
Task: "接入持续麦克风音频流，验证 VAD 可区分说话/静音/噪声状态"
